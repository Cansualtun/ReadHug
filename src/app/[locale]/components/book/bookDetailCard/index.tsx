                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="flex flex-col gap-4">
                        <div className="relative">
                            <Image
                                src={book?.bookId?.images?.thumbnail ?? "/assets/book-placeholder.png"}
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div>
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                <p className="text-xl text-default-700">{book?.bookId?.authors.map((i: any) => i.name).join(" & ")}</p>
                                <span className="text-default-700">•</span>
                                <p className="text-default-700">{book?.bookId?.publishers.map((i: any) => i.name).join(" & ")}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="space-y-2">
                                <p className="text-default-700">
                                    <span className="font-semibold">Yayın Yılı:</span> {book?.bookId?.publishedDate}
                                </p>
                                <p className="text-default-700">
                                    <span className="font-semibold">ISBN:</span> {book?.bookId?.ISBNS[0].identifier ?? ""}
                                </p>
                            </div>
                            <div className="space-y-2">
                                <p className="text-default-700">
                                    <span className="font-semibold">Sayfa Sayısı:</span> {book?.process?.pageCount}
                                </p>
                                <p className="text-default-700">
